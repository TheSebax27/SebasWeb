...
function kgToLbs(kg: number) { return +(kg * KG_TO_LBS).toFixed(1); }
function lbsToKg(lbs: number) { return +(lbs * LBS_TO_KG).toFixed(1); }
function fmtPeso(valor: number, unidad: 'kg' | 'lbs') {
  if (unidad === 'kg') return { principal: `${valor} kg`, conversion: `${kgToLbs(valor)} lbs` };
  return { principal: `${valor} lbs`, conversion: `${lbsToKg(valor)} kg` };
}

/* ── Estado inicial de formularios ── */
const FR = { nombre: '', descripcion: '' };
const FD = { nombre: '' };
const FE = { nombre: '', series: '3', reps: '10', peso: '', unidad: 'kg' as 'kg' | 'lbs', notas: '' };

/* ── Botón toggle kg / lbs ── */
function UnitToggle({ value, onChange }: { value: 'kg' | 'lbs'; onChange: (u: 'kg' | 'lbs') => void }) {
  return (
    <div className="flex rounded-lg overflow-hidden border border-gray-700 h-9 shrink-0">
      {(['kg', 'lbs'] as const).map(u => (
        <button key={u} type="button" onClick={() => onChange(u)}
          className={`px-3 text-xs font-semibold cursor-pointer transition-colors duration-150 ${value === u ? 'bg-emerald-500 text-gray-950' : 'bg-transparent text-gray-500 hover:text-gray-300'}`}>
          {u}
        </button>
      ))}
    </div>
...
export function Gym() {
  ...
  async function crearRutina() {
    ...
    const { data, error } = await supabase.from('rutinas')
      .insert({ nombre: formR.nombre.trim(), descripcion: formR.descripcion.trim() || null })
      .select().single();
    setGuardando(false);
  ...
  async function crearDia() {
    ...
    const { data, error } = await supabase.from('dias_rutina')
      .insert({ rutina_id: rutinaSelId, nombre: formD.nombre.trim(), orden: dias.length })
      .select().single();
    setGuardando(false);
  ...
  async function crearEj() {
    ...
    const { data, error } = await supabase.from('ejercicios_rutina').insert({
      ...
      series: parseInt(formE.series) || 3,
      reps: formE.reps.trim() || '10',
      peso_kg: formE.peso ? parseFloat(formE.peso) : null,
      unidad: formE.unidad,
      notas: formE.notas.trim() || null,
  ...
  function iniciarEdit(ej: EjercicioRutina) {
    ...
    setEditData({
      ...
      reps: ej.reps,
      peso: ej.peso_kg != null ? String(ej.peso_kg) : '',
      unidad: ej.unidad ?? 'kg',
      notas: ej.notas ?? '',
    });
    setShowFE(false);
  ...
  async function guardarEdit(id: string) {
    ...
    const { error } = await supabase.from('ejercicios_rutina').update({
      ...
      series: parseInt(editData.series) || 3,
      reps: editData.reps.trim() || '10',
      peso_kg: editData.peso ? parseFloat(editData.peso) : null,
  ...
  /* ── Formulario de peso (reutilizado en crear y editar) ── */
  function CampoPeso({ valor, unidad, onChange }: {
    valor: string;
    unidad: 'kg' | 'lbs';
    onChange: (campo: 'peso' | 'unidad', v: string) => void;
  }) {
    const num = parseFloat(valor);
    const preview = !isNaN(num) && num > 0
      ? (unidad === 'kg' ? `= ${kgToLbs(num)} lbs` : `= ${lbsToKg(num)} kg`)
      : null;

    return (
      <Field label="Peso">
        <div className="flex gap-2 items-center">
          <input className={inputCls} type="number" placeholder="—" style={{ width: 80 }}
            value={valor} onChange={e => onChange('peso', e.target.value)} />
          <UnitToggle value={unidad} onChange={u => onChange('unidad', u)} />
          {preview && <span className="text-[11px] text-gray-600 whitespace-nowrap">{preview}</span>}
        </div>
      </Field>
    );
  }
                  ...
                  {diaSelId && (
                    <div className="flex flex-col gap-2">
                      ...
                      {ejercicios.map((ej, idx) => {
                        ...
                        const pesoInfo = ej.peso_kg != null ? fmtPeso(ej.peso_kg, unidad) : null;
                            ...
                            {editId === ej.id ? (
                              <div className="border border-dashed border-emerald-900 bg-emerald-950/20 rounded-xl p-4 flex flex-col gap-3">
                                ...
                                <div className="flex gap-2 flex-wrap items-end">
                                  <Field label="Series">
                                    <input className={inputCls} type="number" style={{ width: 70 }} value={editData.series} onChange={e => setEditData({ ...editData, series: e.target.value })} />
                                  </Field>
                                  <Field label="Reps">
                                    <input className={inputCls} placeholder="10 ó 8-12" style={{ width: 90 }} value={editData.reps} onChange={e => setEditData({ ...editData, reps: e.target.value })} />
                                  </Field>
                                  <CampoPeso valor={editData.peso} unidad={editData.unidad}
                                    onChange={(campo, v) => setEditData(d => ({ ...d, [campo]: campo === 'unidad' ? v as 'kg' | 'lbs' : v }))} />
                                </div>
                                <Field label="Notas (opcional)">
                                  <input className={inputCls} value={editData.notas} onChange={e => setEditData({ ...editData, notas: e.target.value })} />
                                </Field>
                              ...
                              <div className="flex items-center gap-3 bg-gray-800 rounded-lg px-4 py-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-2 flex-wrap">
                                    ...
                                    {pesoInfo && (
                                      <>
                                        <span className="text-emerald-400 font-medium">{pesoInfo.principal}</span>
                                        <span className="text-gray-600">·</span>
                                        <span className="text-gray-500">{pesoInfo.conversion}</span>
                                      </>
                                    )}
                      ...
                      {showFE ? (
                        <div className="border border-dashed border-emerald-900 bg-emerald-950/20 rounded-xl p-4 flex flex-col gap-3 mt-1">
                          ...
                          <div className="flex gap-2 flex-wrap items-end">
                            <Field label="Series">
                              <input className={inputCls} type="number" style={{ width: 70 }} value={formE.series} onChange={e => setFormE({ ...formE, series: e.target.value })} />
                            </Field>
                            <Field label="Reps">
                              <input className={inputCls} placeholder="10 ó 8-12" style={{ width: 90 }} value={formE.reps} onChange={e => setFormE({ ...formE, reps: e.target.value })} />
                            </Field>
                            <CampoPeso valor={formE.peso} unidad={formE.unidad}
                              onChange={(campo, v) => setFormE(f => ({ ...f, [campo]: campo === 'unidad' ? v as 'kg' | 'lbs' : v }))} />
                          </div>
                          <Field label="Notas (opcional)">
                            <input className={inputCls} placeholder="Ej. agarre supino" value={formE.notas} onChange={e => setFormE({ ...formE, notas: e.target.value })} />
                          </Field>
                          <div className="flex gap-2">
